...
export class ProductService extends BaseService {
  ...
  async createProduct(productData: Prisma.ProductCreateInput): Promise<Product> {
    try {
      ...
      await this.logActivity(productModel, ActionType.CREATE, productData.createdBy as number, null)
  ...
  async getLowStockProducts(threshold = 10): Promise<Product[]> {
    try {
      ...
      logger.error({ err: error, threshold }, `Error getting low stock products with threshold: ${threshold}`)
      throw error
    }
  }
  ...
  async getProductActivityLogs(productId: number): Promise<any[]> {
    try {
      ...
      logger.error({ err: error, productId }, `Error getting activity logs for product ${productId}`)
      throw error
    }
  }
}
