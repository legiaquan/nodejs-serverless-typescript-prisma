...
export const authRateLimiter = rateLimit({
  ...
  handler: (req: Request, res: Response) => {
    logger.warn({ ip: req.ip, email: req.body.email || "unknown" }, "Authentication rate limit exceeded")
    throw new TooManyRequestsError("Too many authentication attempts, please try again after 15 minutes")
  },
  keyGenerator: (req: Request) => {
    // Use IP address as the key
